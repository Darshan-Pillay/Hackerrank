...
vector<int> freqQuery(vector<vector<int>> queries) {
    ...
    for (size_t queryIndex = 0; queryIndex < queries.size(); queryIndex++) {
       int operation = queries[queryIndex][0];
       int queryInt = queries[queryIndex][1];
       
       if(operation == 1){
           freqFreqMap[intFreqMap[queryInt]]--;
           intFreqMap[queryInt]++;
           freqFreqMap[intFreqMap[queryInt]]++;
       }
       else if(operation == 2 && intFreqMap[queryInt] > 0) {
           freqFreqMap[intFreqMap[queryInt]]--;
           intFreqMap[queryInt]--;
           freqFreqMap[intFreqMap[queryInt]]++;
       }
       else if(operation == 3){
           if (freqFreqMap[queryInt] > 0) {
